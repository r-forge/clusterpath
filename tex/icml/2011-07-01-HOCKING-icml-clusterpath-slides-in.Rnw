...
plot.one <- function(S){
ggplot(subset(cvx,s>=S),aes(alpha.2,alpha.1))+
  geom_path(aes(group=row),colour="black",lwd=1)+
  facet_grid(gamma~norm,labeller=normweights)+
  ##geom_point(aes(size=s,colour=gamma),pch=21)+
  ##geom_path(aes(group=interaction(row,gamma),colour=gamma))+
  ##facet_grid(~norm,labeller=normweights)+
  scale_colour("$\\gamma$")+
  coord_equal()+
  scale_size("$s$")+
